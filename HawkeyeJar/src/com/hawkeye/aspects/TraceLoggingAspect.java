...
public class TraceLoggingAspect {
	...
	public Object trace(ProceedingJoinPoint call, boolean flag)
		...
		if (flag) {
			...
			if (!Monitor.getStatisticsCollectorOfThisThread().isFirstRequest()) {
				Monitor.getStatisticsCollectorOfThisThread().setFirstRequest(true);
				firstRequest = true;
			}
			trace(logger, true, call, null, 0, false);
			Date d = new Date();
			...
			trace(logger, false, call, point, executionTime, firstRequest);
	...
	public void trace(Logger logger, boolean entry, ProceedingJoinPoint call,
			Object retVal, long time, boolean firstRequest) {
		try {
			String action = "";
			String paramList = "";
			if (entry) {
			...
			addToAggregator(call.toLongString(), paramList, action,
					String.valueOf(retVal), time, firstRequest);
	...
	private void addToAggregator(String methodname, String paramList,
		...
		Monitor.getStatisticsCollectorOfThisThread().addTraceBean(bean);
		if (firstRequest) {// add other methods
			JSONAspectRequest req = Monitor.getStatisticsCollectorOfThisThread().getReq();
			JSONAspectRequest newReq = new JSONAspectRequest(Monitor.getStatisticsCollectorOfThisThread().genRequestId());
