...
@Aspect
public class ServletEventsAspects {
	
	
	
	
	@Around(" (execution(public * javax.servlet.http.HttpServlet+.*(..)) || execution (public * javax.servlet.Filter+.*(..)))"
			+ "&& !within(com.hawkeye.*..*) && !within(com.hawkeye.db..*)")
	...
	public void print(Logger logger, boolean entry, ProceedingJoinPoint call,
		...
		try {
			if (entry) {
				...
				logger.info("TRACE Servlet:: ENTERING >> ******************************" + call.toLongString()
						+ " with param : [" + paramList + "]");
			} else {
				logger.info("TRACE Servlet :: EXITING << ************************[" + call.toLongString()
