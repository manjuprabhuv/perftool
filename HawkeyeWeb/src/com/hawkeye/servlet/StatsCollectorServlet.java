...
public class StatsCollectorServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (requeststr.contains("Envelope")) {
				handleWSStats(requeststr);
			} else if (requeststr.contains("requestId")) {
	...
	private void handleWSStats(String requeststr) {
		Gson gson = new Gson();
		WebserviceStatsBean jsonreq = gson.fromJson(requeststr,
				WebserviceStatsBean.class);
		StatsPersistance.addWSStatsToDB(jsonreq.getTimestamp(),
				jsonreq.getRequestId(), jsonreq.getRequest(),
				jsonreq.getResponse());
	}
	...
	private void handleVitalStats(String requeststr) {
		...
		for (HawkEyeMemoryBean memoryUsage : heapMemory) {
			// memoryUsage.ge
			StatsPersistance.addVitalStatsToDB("HeapMemory",
					memoryUsage.getTs(), memoryUsage.getInit(),
					memoryUsage.getUsed(), memoryUsage.getComitted(),
					memoryUsage.getMax());
		}
		List<HawkEyeMemoryBean> nonHeapMemory = memoryMap.get("NonHeapMemory");
		for (HawkEyeMemoryBean memoryUsage2 : nonHeapMemory) {
			// memoryUsage.ge
			StatsPersistance.addVitalStatsToDB("NonHeapMemory",
					memoryUsage2.getTs(), memoryUsage2.getInit(),
					memoryUsage2.getUsed(), memoryUsage2.getComitted(),
					memoryUsage2.getMax());
		}
